...
public static class ShazamApi {
    ...
    static void PopulateAttributes(JsonElement rootElement, ShazamResult result) {
        ...
        // yoni56, 24082024
        if(shazamSongElement.TryGetProperty("meta", out var metaElement)) {

            if(metaElement.TryGetProperty("duration", out var durationElement)) {
                result.DurationSec = durationElement.GetDouble();
            }

            if(metaElement.TryGetProperty("matchOffset", out var matchOffsetElement)) {
                result.MatchOffsetSec = matchOffsetElement.GetDouble();
            }
        }
